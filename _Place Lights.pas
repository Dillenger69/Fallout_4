...
const
    ...
    _exceptionCountMax = 100;   // number of exceptions allowed before we exit the script
	...
	_skipInterval = 2;				// skip every other light placement
var
    ...
    _outputCount, _exceptionCount, _recordCount, _recordsFound, _itemsPlaced, _skipCounter: Integer;  	// current count of sOutput uses and exceptions caught.
...
function Initialize: Integer;
    begin
        ...
        // the tally of records we've gone to place, but skipped or not because of _skipCounter mod _skipInterval = 0
        _skipCounter := 0;
...
function PlaceLight(e: IInterface): integer;
    ...
    begin
        if(_itemsPlaced < _itemsPlacedMax) then // if we've placed max items, Exit with Result := 1
        begin
            if(_skipCounter mod _skipInterval = 0) then // only place at the _skipInterval defined at the top. 2 would place every other light.
			begin
				//---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
				...
				try                
					SetElementNativeValues(newLight, 'XRDS', 20);                              // Lets try the lights at 20
				...
				try
					...
					on Ex: Exception do
					begin
						Result := LogException(Ex,'Caught modifying the new light CELL to: ' + sCell);
						Exit;
					end; // end on Ex
				end; // end try/except
				
				//---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
				// Lets see if we can turn on LOD for the lights
				try
					SetIsVisibleWhenDistant(newLight, True);
				except
					on Ex: Exception do
					begin
						Result := LogException(Ex,'Caught trying to set LOD True on newLight.');
						Exit;
					end; // end on Ex
				end; // end try/except
			...
			_skipCounter := _skipCounter + 1; // update the skip counter whether we place an item or not.
        end // end if
        else
        begin
            //---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
